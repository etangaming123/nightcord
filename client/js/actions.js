...
import {
  can, currentChannel, currentGuild, ensureReadState, isDm, isGuildOwner, isUnread, markRead, memberById, pref,
  rememberUser, resetMessages, sortChannels, state, userById,
} from "./state.js";
...
import { $, displayName, idGt } from "./ui/dom.js";
import { openEmojiPicker } from "./ui/emoji.js";
import { guildSettings } from "./ui/guildSettings.js";
import { closeFullscreen, closeModal, confirmModal, fullscreenOpen, openMenu, refreshFullscreen, toast } from "./ui/modals.js";
...
export async function sendMessage(content, reply) {
  ...
  const res = await req(T.MESSAGE_SEND, payload);
...
export function jumpTo(messageId) {
  if (!flashMessage(messageId)) toast("That message isn't loaded — scroll up to find it.");
}
...
export const createInvite = async () => (await req(T.GUILD_INVITE_CREATE, { guild_id: state.guildId })).invite_code;
...
export async function guildMenu(g, anchor) {
  ...
  openMenu(anchor, [
    ...
    can("CREATE_INVITE") ? { label: "Invite people", icon: "✉", onClick: () => openGuildSettings("invites") } : null,
...
export const createChannel = () => dialogs.createChannelDialog({
  roles: state.roles,
  canSetPerms: can("MANAGE_ROLES"),
  onSubmit: async (name, overwrites) => {
    const res = await req(T.CHANNEL_CREATE, { guild_id: state.guildId, name, overwrites });
...
export async function moveChannel(channel, delta) {
...
export function channelMenu(c, anchor) {
  ...
  const i = state.channels.findIndex((x) => x.channel_id === c.channel_id);
  const manage = can("MANAGE_CHANNELS", c);
  openMenu(anchor, [
    { label: "Mark as read", icon: "✓", disabled: !isUnread(c.channel_id), onClick: () => markChannelRead(c.channel_id) },
    { label: p.muted ? "Unmute channel" : "Mute channel", icon: p.muted ? "🔔" : "🔕", onClick: () => setNotifyPref(c.channel_id, { level: p.level, muted: !p.muted }) },
    ...levelItems(c.channel_id, { inherit: "Use guild default" }),
    manage ? "-" : null,
    manage ? { label: "Edit channel", icon: "⚙", onClick: () => openChannelSettings(c) } : null,
    manage && i > 0 ? { label: "Move up", icon: "↑", onClick: () => moveChannel(c, -1) } : null,
    manage && i < state.channels.length - 1 ? { label: "Move down", icon: "↓", onClick: () => moveChannel(c, 1) } : null,
    manage ? { label: "Delete channel", icon: "🗑", danger: true, onClick: () => dialogs.deleteChannelDialog(c, () => req(T.CHANNEL_DELETE, { channel_id: c.channel_id })) } : null,
    "-",
...
export function moderationItems(userId) {
  const m = memberById(userId);
  if (!m || state.view !== "guild" || !outranks(userId)) return [];
...
export const actions = {
  ...
  addGuild, ghostJoin, updateGuild, createInvite, leaveGuild, guildMenu, guildSettings: openGuildSettings,
