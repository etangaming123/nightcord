...
import { closePopover, openMenu, openPopover } from "./modals.js";
...
function accountRow(server, account, actions) {
  ...
  iconBtn("⋯", t("account_options"), (e) => openMenu(e.currentTarget, [
    ...
    { label: current ? t("log_out") : t("forget"), icon: "🚪", danger: true, onClick: () => { closePopover(); actions.forgetAccount(server.url, account.user_id); } },
  ].filter(Boolean), { placement: "right" })));
}
...
export function openAccountSwitcher(anchor, actions) {
  ...
  openPopover(anchor, body, { placement: "top", cls: "accounts-pop" });
}
