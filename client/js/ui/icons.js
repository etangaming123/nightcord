// Interface icons: inline SVG so the hosted client and the single-file
// standalone build both work without fetching anything. Line icons are from
// Lucide (https://lucide.dev, ISC licence, see LICENSE); the status-* shapes
// are our own. Emoji stay emoji; these are for buttons, menus and labels.
...
// Filled status shapes, coloured by CSS (.ico.status-*).
const FILLED = {
  "status-online": '<circle cx="12" cy="12" r="9"/>',
  "status-idle": '<path d="M20.5 14.5A9 9 0 1 1 9.5 3.5a7 7 0 0 0 11 11z"/>',
  "status-dnd": '<path fill-rule="evenodd" d="M12 3a9 9 0 1 1 0 18 9 9 0 0 1 0-18zM7 10.5a1.5 1.5 0 0 0 0 3h10a1.5 1.5 0 0 0 0-3z"/>',
  "status-invisible": '<path fill-rule="evenodd" d="M12 3a9 9 0 1 1 0 18 9 9 0 0 1 0-18zm0 4.5a4.5 4.5 0 1 0 0 9 4.5 4.5 0 0 0 0-9z"/>',
};

const cache = new Map();

export const hasIcon = (name) => typeof name === "string" && (name in LUCIDE || name in FILLED);

// <svg class="ico ico-{name}"> for a known icon name; aria-hidden, since the
// button or menu item around it carries the label.
export function icon(name, { cls = "" } = {}) {
  let tpl = cache.get(name);
  if (!tpl) {
    const filled = name in FILLED;
    const body = filled ? FILLED[name] : LUCIDE[name];
    if (body === undefined) throw new Error(`unknown icon ${name}`);
    const paint = filled
      ? 'fill="currentColor" stroke="none"'
      : 'fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"';
    const t = document.createElement("template");
    t.innerHTML = `<svg class="ico ico-${name}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" ${paint} aria-hidden="true" focusable="false">${body}</svg>`;
